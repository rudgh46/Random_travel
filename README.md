...
TAGO 실제 배차 스위트(20개)는 `TAGO_KEY` 환경변수가 있을 때만 돌아가고,
없으면 건너뜁니다.
...
> **터미널 ID 는 이름당 하나가 아닙니다.** 같은 이름에 여러 ID 가 있고 노선에 따라
> 유효한 쪽이 다릅니다. 센트럴시티(서울)는 `NAEK020`·`NAEK021` 두 개인데
> 목포행은 `NAEK020` 에서 0편, `NAEK021` 에서 15편입니다(동서울은 네 개).
> 그래서 `mode=route` 는 이름으로 찾은 후보 ID 를 순회해 결과가 나오는 조합을
> 채택하고, 응답의 `tried` 에 어떤 조합을 몇 편으로 확인했는지 남깁니다.
