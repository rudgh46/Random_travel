...
승차권에는 **목적지의 오늘 날씨**(상태·현재기온·최저/최고·강수확률)가 함께
표시됩니다. [Open-Meteo](https://open-meteo.com) 를 쓰며 **인증키가 필요 없고
CORS가 열려 있어** 프록시 없이 브라우저에서 직접 호출합니다. 강수확률이 60%
이상이면 우산 안내가 붙고, 조회에 실패하면 그 줄만 조용히 숨깁니다.
