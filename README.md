...
> **배포 후 한 가지.** `index.html` 의 `og:image` 가 `/og.png` (루트 상대 경로)로
> 되어 있습니다. 카카오톡·트위터 등 일부 크롤러는 **절대 URL만** 인식하므로,
> 도메인이 정해지면 `https://<도메인>/og.png` 로 바꿔 주세요.
> 바꾸지 않아도 사이트 동작에는 영향이 없고, 미리보기 이미지만 안 보일 수 있습니다.
