...
이어집니다. 99개 목적지 전체에 대해 내장 데이터로 동작하며 별도 API 키가 필요 없습니다.
...
지점을 한 번에 돌려주므로, 99곳 전체를 **요청 1회**로 받아 걸러냅니다.
...
당일치기를 계획할 때 실제로 궁금한 건 **서울행 막차**입니다. TAGO 는 역방향
조회도 되므로 목적지 → 서울 노선을 함께 싣습니다.

승차권과 노선 페이지에 `서울행 막차 22:00 · 현지에서 10시간 13분 · 하루 23편`
이 표시되고, 당일치기 판정이 **막차 기준 실제 계산**으로 바뀝니다.
...
예전에는 "소요시간 150분 이하면 당일치기" 라는 추측이었습니다. 지금은 출발
시각을 바꾸면 판정이 함께 바뀝니다(강릉 09:00 출발은 당일치기, 18:00 출발은 1박).

> **12곳은 역방향 자료가 아예 없습니다.** 가는 편은 있는데 오는 편만 없습니다
> (예: 서울→동송 4편, 동송→서울 0편). 서울 터미널 후보 네 곳을 모두 시도해도
> 0편이라 우리 쪽에서 보정할 수 없습니다. 시외버스로 운행하는 구간으로 보이며,
> 이 목적지들은 그 줄을 감추고 코버스·시외버스포털로 안내합니다.
>
> 고한·사북 횡성 증평 괴산 보은 속리산 울진 봉화 신철원 동송 운천 부안

막차·편수는 `index.html` 의 `RETURN` 에 `[출발터미널ID, 서울도착터미널ID, 막차, 편수]`
로 들어 있습니다. **요일과 시기에 따라 달라지는 값**이라 조회 시점 기준이며,
노선 페이지에서는 그날 실제 배차로 덮어씁니다. 낡았는지 확인하려면
[`tests/audit-return.js`](tests/audit-return.js) 로 전수 점검합니다.

```
TAGO_KEY="키" node audit-return.js
```
...
목적지·요금 데이터를 손봤다면 전수 점검도 함께 돌리세요. 99곳이 실제로
...
경부·영동선 + 호남선(센트럴시티) 총 99개 노선의 우등 기준 편도
예상 요금·소요시간을 내장했습니다. 2026-08-06 에 22곳을 추가했고, 추가분은
TAGO 실측값(우등 최저 요금·소요시간 중앙값)을 그대로 넣었습니다.
서울 두 터미널에서 갈 수 있는 곳은 180곳이지만, 나머지는 이미 있는 도시의
세부 정류소(평택 4개, 아산 계열 7개 등)나 고속도로 환승 정류소여서
랜덤 목적지로 적절하지 않아 제외했습니다. 실제 출발편·잔여석은 코버스(kobus.co.kr)에서
...
목적지 36곳)를 조합해 만듭니다. 필터 4종은 브라우저에 저장하지만
...
권역은 도 단위로 나눕니다 — 수도권, 강원, 충북, 충남·세종·대전, 전북,
전남·광주, 경북·대구, 경남·부산·울산. 이전에는 "충청·세종"과 "충청 서해안"
으로 갈라 청주·충주가 서해안에 묶이는 등 기준이 모호했습니다.
